...
def run(episodes, hp, is_training=True, render=False):
    ...
    for i in range(episodes):
        ...
        cnt_v = np.zeros(hp['pos_space_seg']) # number of times the velocity exists at each position state for current episode
        ...
        while (not terminated and rewards > -1000):
            ...
            if is_training:
                ...
                cnt_v[new_state_p] += 1
                # v2[new_state_p, i] = v2[state_p, i] + (action - 1) * force - math.cos(3 * state_p) * gravity
                # v2[new_state_p, i] = np.clip(v2[new_state_p, i], -0.07, 0.07) # clip to [-0.07, 0.07]
            # update the state
            state = new_state
            state_p = new_state_p
            state_v = new_state_v
        ...
        # avg_v = np.divide(sum, cnt, where=cnt!=0, out=np.zeros_like(sum))
        # history[i, :] = avg_v
