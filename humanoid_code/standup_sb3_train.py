...
if not os.path.exists(path):
    os.makedirs("./model/Humanoid", exist_ok=True)
    
## 5. Train the model
TOTAL_TIMESTEPS = 25000
for i in range(15):
    # 1) define the callback
    eval_callback = EvalCallback(env, best_model_save_path='./best_models/Humanoid/'+RL_NAME+'/', log_path='./logs/', verbose=1, eval_freq=1000)
    # checkpoint_callback = CheckpointCallback(save_freq=1000, save_path='./logs/', name_prefix='PPO')
    # 2) train the model
    model.learn(total_timesteps=TOTAL_TIMESTEPS, callback=eval_callback)
    # model.learn(total_timesteps=TOTAL_TIMESTEPS, callback=eval_callback, reset_num_timesteps=False)
    # 3) save the model
    model.save(f"./model/Humanoid/{RL_NAME}_Humanoid_{TOTAL_TIMESTEPS*(i+1)}.pkl")
